...
shinyServer(function(input, output, session) {
  ...
  output$full_table <- renderDataTable({
    datatable(dataset, selection = 'single', options = list(pageLength = 10))
  })
  
  # Render the short data table with only two columns
  output$short_table <- renderDataTable({
    datatable(dataset[, c(1, 2)], selection = 'single', options = list(pageLength = 10)) # Assuming columns 1 and 2 are needed
  })
